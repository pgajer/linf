...
Compared with clustering-based CSTs, dCSTs replace the
algorithm/distance/initialization/$k$ choices with an explicit retained
feature family and support policy, giving a microbiome workflow for
defining, transferring, rebuilding, and testing deterministic state panels.

We evaluate dCSTs in two complementary microbiome applications. In gut data, we
learn a shared-taxonomy dCST vocabulary in a large American Gut Project
background cohort and test its portability in external inflammatory bowel
disease cohorts. Direct transfer and rebuilt-cohort analyses recover clinically
coherent sample-level IBD structure, especially in Halfvarson 2017, while
subject-aware sensitivity analyses show that portability weakens under
repeated-sample dependence and cohort mismatch; this application is therefore a
portability demonstration, not broad IBD disease-state validation. In a
separate vaginal metagenomic application, dCSTs organize full-VOG gene-family
states associated with \textit{Chlamydia trachomatis} clearance and persistence
within the labeled CT cohort; support comes from cap-based robustness rather
than from an independent CT replication cohort, together with biologically
readable taxonomic, product-level, and KEGG-linked follow-up.

The applications evaluate dCSTs as a state-representation layer rather than as a
universal biomarker vocabulary: the framework provides deterministic fitted
state vocabularies and frozen sample assignments for cross-cohort portability
analysis and local biological interpretation.
\end{abstract}
...
explicit retained-feature lineages for aligned samples. We treat dCSTs here as
a state-representation layer rather than as a universal biomarker vocabulary.
...
CT application tests within-cohort interpretability: whether the dCST
construction remains biologically readable in a different niche and feature
universe, using full-VOG gene-family states with taxonomic, product-level, and
KEGG-linked follow-up. Across the two settings, the analyses examine three
properties of dCSTs: deterministic and interpretable state construction without
clustering-defined labels; cross-cohort portability under compatible conditions
in gut IBD; and within-cohort metagenomic interpretability in an independent CT
setting.
...
\]
with ties resolved by the deterministic feature-dictionary-order tie rule
(implemented as \texttt{tie.method = "first"} in our reference implementation),
which selects the first retained feature in the fixed feature-dictionary order
when more than one feature attains the within-sample maximum. Retained
...
\midrule
Input abundance matrix & A non-negative sample-by-feature matrix \(X\). The input may be counts, proportions, or another sample-wise transformation, provided the within-sample order of retained features is preserved; examples include strictly monotone transformations applied featurewise within a sample. CLR preserves order for strictly positive compositions, but any zero-replacement or pseudocount step must also preserve within-sample order for the same pre-policy lineage to be guaranteed. \\
...
strictly monotone featurewise transformations such as \(\log(1+x)\) or
...
Implementation-level settings — tie rule, Monte Carlo seed and simulation
count, Bayesian draw count, and the BH families used in each contrast — are
...
explicit modeling choices. The gut support-schedule audit therefore reports how
\(\pm25\%\) threshold perturbations change pre-absorb lineage granularity, while
the CT capped-VOG analyses evaluate robustness to nested retained-feature caps.
...
design lets the same dCST construction support application-specific inference
without requiring identical statistical designs.
...
CT (vaginal metagenomics) & Independent application with full-VOG hierarchies learned in a 3,602-sample CT+reference cohort; outcome testing restricted to 604 labeled CT samples. & The full-VOG hierarchy reached its strongest panel-level CT association at depth~4 ($q = 0.0047$, $V = 0.371$), capped VOG runs showed support stabilizing from 50k features onward, and the leading branches were biologically interpretable through taxonomic, product-level, and KEGG-linked follow-up. & dCSTs can remain taxonomically and functionally interpretable across a different niche and feature universe without implying external CT replication. \\
\bottomrule
\end{tabularx}
...
\url{https://github.com/pgajer/linf} and installable in R~$\geq$~4.0
with \texttt{remotes::install\_github("pgajer/linf@v0.1.0")}. A tagged
release of \texttt{linf} v0.1.0 archived on Zenodo is the canonical
reference for this manuscript; the Zenodo DOI is listed in Data and code
availability alongside the corresponding repository commit. The package
provides the core public operations used in the manuscript:
\texttt{normalize.linf()} for sample-wise normalization,
\texttt{linf.cells()} for dominant-feature assignment,
\texttt{linf.csts()} for depth-1 dCST construction with pure or absorb
low-support policies, \texttt{refine.linf.csts()} and
\texttt{refine.linf.csts.iter()} for recursive depth-wise refinement, and
\texttt{transfer.dcsts()} for assigning aligned samples into a frozen fitted
hierarchy. In terms of Algorithm~\ref{alg:dcst-fit-transfer}, the
depth-1 step of the Fit block is implemented by \texttt{linf.cells()}
followed by \texttt{linf.csts()}, the depth-$\ell>1$ recursion together
with absorb reassignment is implemented by \texttt{refine.linf.csts()}
applied iteratively via \texttt{refine.linf.csts.iter()}, and the
Frozen-transfer block is implemented by \texttt{transfer.dcsts()}. The
manuscript analyses use these package functions together with
application-specific scripts for cohort filtering, rebuilt-cohort analysis,
association testing, and biological follow-up. Repository locations and build
identifiers are listed in Data and code availability and Supplementary
Tables~S1--S4.
...
($q = 0.042$, bootstrap recurrence 49.6\%, just below the 50\% pragmatic
working threshold for subject-level stability used in the source gut analysis),
whereas the corresponding AGP-transferred labels attenuated. This asymmetry indicates that
...
cohort. Across these cohorts, dCSTs provide an interpretable taxonomic state
vocabulary that transfers usefully into compatible external IBD data, while
transfer strength depends on cohort comparability, sample type, and
repeated-subject structure; the result is therefore a demonstration of portable
representation under compatible conditions, not broad validation of an IBD
disease-state taxonomy (Figure~\ref{fig:gut-overview}).
...
instead, it asks whether dCSTs can organize vaginal metagenomic outcome
structure in a biologically interpretable form. The broad
...
58 dominance-lineages by 2 outcome columns). These results indicate that CT
outcome is organized across a gene-family-defined dCST panel rather than by a
single deterministic VOG biomarker; consistent with this framing, the leading
single labels do not pass label-level BH correction at the same depth (best
...
broader, even as the panel-level signal stabilized. This identity shift is the
operational reason we interpret the CT result at the panel and
dominance-lineage level rather than at the single-label level. The CT result is
...
Two framework-relevant lessons emerge from the CT analysis. First, panel-level
inference is more stable than single-label inference at high feature
dimension: the omnibus depth-4 panel-level signal is reproducible from the
50k-feature cap onward, whereas the identity of the single top-ranked
direction-specific label is not (see below). Second, the absorb policy keeps
biologically interpretable named dominance-lineages at depth~4 in a
509,501-feature universe rather than collapsing low-support refinements into
one residual bucket; this is what makes the depth-4 clearance and
persistence-oriented branches readable through taxonomic, product-level, and
KEGG-linked follow-up. Both lessons generalize beyond the CT setting and are
properties of the dCST construction itself.
...
The two applications probe complementary framework-level properties: gut tests
portability of a deterministic taxonomic state vocabulary across external
cohorts, while CT tests within-cohort biological readability across a much
larger feature universe. Both applications show panel-level signal that
strengthens with refinement depth, although the two analyses use different
tests and different effect-size scales
(Figure~\ref{fig:cross-app-synth}; Supplementary
Figure~\ref{fig:supp-panel-signal-synthesis}). The shared conclusion is
structural rather than label-level: deterministic assignment and interpretable
dominance-lineages give a stable substrate for microbiome state analysis,
while portability, robustness, and biological interpretability remain
application-specific empirical properties.
...
\caption{Cross-application synthesis. The gut application evaluates whether
an AGP-derived taxonomic dCST vocabulary transfers into external IBD cohorts
and how that signal changes under subject-aware sensitivity; the CT
application evaluates whether full-VOG dCSTs organize clearance- and
persistence-associated metagenomic states with taxonomic and functional
follow-up. The shared conclusion is framework-level rather than
universal-label.}
...
This study asks whether microbiome state labels can be made deterministic and
...
empirical cross-tabulation between dCSTs and VALENCIA-CSTs is therefore out of
scope for this paper: VALENCIA's centroids are trained on 16S amplicon data,
...
approaches are useful when the scientific
goal is cluster discovery, probabilistic mixture modeling, or assignment to a
curated centroid vocabulary in their respective input modalities. dCSTs
address a narrower representation problem:
...
important result is not external transfer, but that dCSTs remain
informative in a different body site, with a different feature universe,
and with a richer biological follow-up layer. The
...
The two analyses validate different claims rather than the same one. The gut
analysis evaluates external transfer and rebuilt-cohort behavior in clinically
annotated IBD datasets, with explicit limits imposed by cohort mismatch and
repeated-subject structure. The CT analysis evaluates within-cohort
gene-family-level outcome interpretation and feature-cap robustness, but does
not provide an external CT replication cohort. Across both, the value of dCSTs
is that they separate state construction from downstream inference: the same
fitted vocabulary serves transfer and rebuilt comparisons in gut, and outcome
association with functional follow-up in CT, without claiming a universal state
vocabulary, a diagnostic classifier, a causal mechanism, or guaranteed
portability in every microbiome setting.
...
The natural extensions follow directly. In gut, subject-aware inference should
be strengthened in cohorts with more unique controls, richer subtype metadata,
and additional molecular layers. In CT, independent vaginal metagenomic cohorts
are needed to test external replication of the leading VOG-defined states, and
...
biological interpretation, particularly in settings where taxonomic,
functional, and clinical data are analyzed jointly rather than separately.
Forward-looking applications consistent with this property include
cross-cohort meta-analysis standardization, pre-specified trial stratification,
and longitudinal monitoring; each requires prospective validation before
clinical use.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{../assets/figures/FIGURE_S3_panel_signal_synthesis.pdf}
\caption{Cross-application synthesis on a single shared axis: panel-level
Cramer's $V$ versus dCST hierarchy depth in the two applications. The CT
full-VOG outcome panel (blue circles, $n_{\mathrm{CT}}=604$) shows
panel-level $V$ rising from 0.214 at depth~1 to 0.371 at depth~4. The AGP
self-reported IBD condition-prioritization screen (red squares) reports
$V = 0.24$ at depth~1 and $V = 0.31$ at depth~4 in the main text; depths~2
and~3 are not reported numerically and the dashed segment is therefore an
endpoint guide rather than an imputation. The two analyses are not the same
kind of test: AGP IBD is an exploratory phenotype screen in the AGP discovery
cohort, whereas CT full-VOG is the primary outcome-association test in the
labeled CT subset. Cohorts, retained-feature universes, and corresponding
$V$ scales differ by design. Halfvarson direct-transfer and rebuilt-cohort
panel-level signal is reported categorically in the main text
(Figure~\ref{fig:gut-overview}) rather than as a depth-resolved $V$ curve and
is therefore not plotted here. The figure documents the qualitative pattern
that panel-level signal in both applications strengthens with refinement,
consistent with the framework-level interpretation of the method.}
\label{fig:supp-panel-signal-synthesis}
\end{figure}
