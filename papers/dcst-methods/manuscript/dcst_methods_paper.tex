...
summaries, but clustering-derived labels can change when the comparison cohort,
distance metric, initialization, or number of clusters changes. We present
...
family, meaning the taxa or gene groups kept for a given analysis. Under the
absorb policy used here, the fitted hierarchy is deterministic for a fixed
reference cohort, retained features, tie rule, and support schedule; once
frozen, it provides stable assignment for aligned cohorts.

We evaluate dCSTs in two microbiome settings. In gut data, a shared-taxonomy
American Gut Project (AGP) dCST vocabulary transfers clinically coherent
sample-level inflammatory bowel disease (IBD) structure into external cohorts,
especially Halfvarson 2017, while subject-aware sensitivity bounds the
portability claim. In vaginal metagenomes, full VIRGO2 orthologous gene group
(VOG) dCSTs organize \textit{Chlamydia trachomatis} (CT) clearance and
persistence within the labeled CT cohort, with support from feature-cap
robustness and taxonomic, product-level, and KEGG-linked interpretation.
Together, the two applications use dCSTs as a deterministic state vocabulary for
portability analysis and local biological interpretation, not as a universal
biomarker set.
\end{abstract}

\section{Introduction}

Microbiome studies often need compact state definitions: labels that summarize
high-dimensional abundance tables using a small set of shared features or a
cohort-level grouping. Community-state types, enterotypes, and related coarse
labels make it possible to compare samples, cohorts, and phenotypes
...
partitioning procedures, so a label learned in one cohort may not have the same
definition, or may not transfer directly, when the comparison cohort, distance
metric, or partitioning rule changes. This makes portability and
reproducibility harder to interpret.
...
\bottomrule
\end{tabular}
\caption{Overview of the fitted dCST hierarchies analyzed in this paper. Cohort $n$ is the
...
then each group of samples with the same current label is split again by the
next dominant retained feature. With the absorb policy used here, small
provisional subgroups created during refinement are reassigned into nearby
retained named states rather than pooled into one residual bucket. The fitted
absorb-policy vocabulary is
...
Gut Project background transfers into clinically annotated inflammatory bowel
disease (IBD) cohorts, how direct transfer compares with cohort-specific
rebuilt hierarchies, and how those signals behave under subject-aware
sensitivity. The vaginal metagenomic \textit{Chlamydia trachomatis} (CT)
application tests within-cohort interpretation: whether the same construction
remains useful in a different niche and feature universe, using full VIRGO2
orthologous gene group (VOG) feature states with taxonomic, product-level, and
KEGG-linked follow-up. Together, the settings test whether dCSTs can provide
deterministic, interpretable state construction without clustering-defined
labels; cross-cohort portability under compatible conditions in gut IBD; and
...
retained feature, and each resulting sample group is then refined by the next
dominant retained feature among samples in that group. The absorb policy
reassigns low-support provisional subgroups into retained named states from the
same refinement step rather than collapsing them into a residual bucket.}
...
labels whose cohort-specific definitions can change with the comparison cohort.

Because recursive refinement produces many provisional low-frequency subgroups,
the hierarchy also requires an explicit low-frequency policy. We use the absorb
policy throughout the primary analyses. Under this policy, provisional subgroups
below the depth-specific minimum-support threshold are reassigned into retained
named states rather than pooled into a single residual bucket.
...
as a second formal BH-significant result. In practical terms, the same
ulcerative-colitis-associated label reappeared in more than half of the
one-sample-per-subject resamples, but the evidence was not strong enough to call
it a second corrected positive result. Halfvarson therefore supports sample-level
portability of the representation, but the repeated-sample structure limits
disease-versus-control transfer interpretation.
...
constructs. In practical terms, this asks whether a sample can receive the same
state label whenever the retained features and reference hierarchy are fixed,
rather than receiving a label that depends on refitting a clustering solution.
...
lineage sets of that hierarchy. This turns a microbiome state
...
\texttt{121659053028f9ab57bab7e8daf0d50cbfdb8555} at \manuscriptbuilddatetime.
