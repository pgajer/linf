...
full-VOG gene-family states associated with \textit{Chlamydia trachomatis}
clearance and persistence within the labeled CT cohort; support comes from
capped VOG robustness and biologically readable taxonomic, product-level, and
...
used for gut taxa and CT full VOG gene groups.}
...
full-VOG dCSTs organize clearance- and persistence-associated metagenomic
states with interpretable taxonomic and functional follow-up. The
...
active backend. If no retained sibling has positive abundance in sample \(i\),
...
and 1,000 simulations per panel in the run reported here. CT full-VOG omnibus
screens used 50,000 Monte Carlo simulations per panel; the CT association
scripts did not set a separate Fisher-test seed, so the generated result tables
are the source of record for exact simulated \(p\)-value digits.
...
CT (vaginal metagenomics) & Independent application with full-VOG hierarchies learned in a 3,602-sample CT+reference cohort; outcome testing restricted to 604 labeled CT samples. & The full-VOG hierarchy reached its strongest panel-level CT association at depth~4 ($q = 0.0047$, $V = 0.371$), capped VOG runs showed support stabilizing from 50k features onward, and the leading branches were biologically interpretable through taxonomic, product-level, and KEGG-linked follow-up. & The same deterministic framework can remain taxonomically and functionally interpretable across a different niche and feature universe without implying external CT replication. \\
\bottomrule
\end{tabularx}
...
applied to full VOG feature families rather than to stool taxa. The labels
therefore live in different biological vocabularies across the two
applications, but the assignment rule is the same.
...
single deterministic VOG biomarker. Within that panel, the most stable lineage
...
chemotaxis- and flagellar-related features. These follow-up annotations show
that the framework can recover outcome-associated states while preserving
taxonomic and functional interpretability at gene-family resolution.
...
This distinction should be read against existing microbiome state frameworks,
not as a claim that dCSTs replace all of them. PAM-style enterotypes summarize
large gut datasets by clustering samples in a distance space
...
\cite{France2020_VALENCIA}. These approaches are useful when the scientific
goal is cluster discovery, probabilistic mixture modeling, or assignment to a
curated centroid vocabulary. dCSTs address a narrower representation problem:
given a retained feature family and support policy, define a deterministic
rank-based state label that can be frozen, transferred, rebuilt, and tested
without choosing a distance-based clustering partition.
...
support full-VOG outcome analysis with capped-feature robustness and biological
interpretation, but it does not provide an external CT replication cohort.
Thus, only the gut application evaluates external transfer; the CT application
evaluates within-cohort metagenomic interpretability and feature-cap
robustness. Together, these
...
replication of the leading VOG-defined states, and targeted mechanistic
follow-up would be needed to evaluate the transport- and
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{../assets/figures/FIGURE_S2_clustering_drift_demo.pdf}
...
seed (20260428). PAM/enterotype-style clustering used Jensen--Shannon distance. Label
stability is summarized by the adjusted Rand index and by the best matched
