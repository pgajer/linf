...
statistical association strength. In this study we use the
...
Bayesian analysis used parallel models to estimate how strongly the data
supported enrichment or depletion of each state, which was especially
useful for rarer deeper states. Multiple testing in the conventional
...
allergies. Independent validation of the absorb hierarchy is being
extended in parallel, including IBD-relevant cohorts carried forward
from the earlier analysis and newly harmonized IBS datasets. All
manuscript figures, tables, and analysis scripts were generated from
workflows built around the \texttt{linf} R package, with
project-specific runners maintained in the companion
\texttt{gut\_microbiome} repository.
...
residual tail. Omnibus phenotype-by-depth
...
\begin{table}[H]
\small
\caption*{\textbf{Table 1.} Top depth-1 absorb DCSTs in the full AGP cohort.}
\centering
\begin{tabular}{@{}lllll@{}}
\toprule
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.9\linewidth}{\textbf{Placeholder for absorb hierarchy overview.}
Insert an absorb-specific figure showing (i) the depth-1 landscape,
(ii) label growth across depths 1--4, and (iii) depth-wise omnibus
effect sizes for the key phenotype branches. A separate pure-versus-
absorb comparison figure should be placed in the supplementary
analyses.}}
\caption{Placeholder for an absorb-hierarchy overview figure.}
\end{figure}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.9\linewidth}{\textbf{Placeholder for IBD absorb lineage figure.}
Insert an absorb-specific lineage panel showing the progression from
depth-1 \taxon{Bacteroides} to the depth-2 through depth-4 inflammatory
lineages, together with Bayesian-supported rare enterobacterial states
such as \taxon{Morganella}.}}
\caption{Placeholder for an IBD absorb-lineage summary figure.}
\end{figure}
...
\subsection{Remaining phenotypes and external data}\label{other-phenotypes-and-external-data}
...
lineage in the Batch 1 follow-up. We keep that branch out of the main
text for now because it is harder to interpret cleanly in a
...
and was not prioritized further after the Batch 1 rerun. Independent
external validation is being extended for the absorb hierarchy,
beginning with IBD-focused cohorts and newly harmonized IBS datasets,
and those analyses will determine how broadly the current discovery
patterns generalize. Supplementary analyses will also carry the full
pure dCST-CT association screens and the direct pure-versus-absorb
comparison so that the main text can stay centered on the absorb
hierarchy.

\begin{figure}[H]
\centering
\fbox{\parbox{0.9\linewidth}{\textbf{Placeholder for absorb-specific external validation summary.}
Insert the rerun external-validation figure and table for the absorb
hierarchy, beginning with the IBD cohorts and the newly harmonized IBS
validation datasets.}}
\caption{Placeholder for an absorb-specific external-validation summary.}
\end{figure}
...
or less common in cases. The two additional Batch 1 examples broaden
...
Several limitations remain important. Absorb-specific external
validation is still being extended, and the present manuscript develops
the lineage-level story most fully for IBS, IBD, autoimmune disease,
acid reflux, and seasonal allergies. AGP phenotypes are also
self-reported and cross-sectional, so residual confounding, medication
effects, and reverse causation remain plausible. Those concerns are
especially important for acid reflux and cardiometabolic phenotypes. The
current evidence nevertheless supports the absorb framework as a
biologically interpretable analysis layer and motivates formal testing
of its generalizability in independent cohorts.
...
\texttt{run\_absorb\_label\_followup.R}. Additional external validation
datasets for the absorb branch are being harmonized in parallel with the
discovery analyses reported here. The corresponding pure dCST-CT
association outputs are retained as supplementary reference analyses.
