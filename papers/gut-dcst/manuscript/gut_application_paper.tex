...
allergies. This manuscript is intentionally centered on the discovery
cohort; absorb-specific external validation remains a planned next step
rather than a completed line of evidence for the present paper. All
...
validation remains future work, and the present manuscript is therefore
best read as a discovery-cohort analysis that develops the lineage-level
story most fully for IBS, IBD, autoimmune disease, acid reflux, and
seasonal allergies. AGP phenotypes are also self-reported and
cross-sectional, so residual confounding, medication effects, and
reverse causation remain plausible. Those concerns are especially
important for acid reflux and cardiometabolic phenotypes. The current
evidence nevertheless supports the absorb framework as a biologically
interpretable analysis layer and motivates formal testing of its
generalizability in independent cohorts.
...
\texttt{run\_absorb\_label\_followup.R}. Supplementary materials for the
present manuscript summarize phenotype definitions and sample counts,
the pure-versus-absorb depth-change summary, and selected reference
analyses retained outside the main text.
