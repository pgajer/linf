# Historical AGP taxonomy and transfer provenance

Date of original analysis: 2026-04-26.

This is a preserved technical record, not a statement of current package
behavior or current manuscript results. The original rerun used a fallback
and carry-forward transfer procedure later corrected as documented in
`AGP_ABSORB_TRANSFER_IMPLEMENTATION_AUDIT_2026-04-26.md`. In particular,
nominal full assignment is not proof of valid frozen-hierarchy membership.
The source identifiers, methods, script names, output locations, and historical
result comparisons below are retained for traceability.
