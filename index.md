...
- **L∞ normalization** —
  ...
  row-wise division by maximum, mapping each sample to the L∞ unit-ball
  boundary.
- **Dominant-feature assignment** —
  [`linf.dominant.features()`](https://pgajer.github.io/linf/reference/linf.dominant.features.md):
  rank-1 assignment per sample, returning indices, labels, and level
  sets.
- **Truncated dCSTs** —
  [`linf.csts()`](https://pgajer.github.io/linf/reference/linf.csts.md):
  apply the support threshold *n*₀ and either group low-support samples
  together or absorb them into retained states.
- **Iterative refinement** —
  [`refine.linf.csts()`](https://pgajer.github.io/linf/reference/refine.linf.csts.md):
  depth-2+ dCSTs via successive rank decomposition, with automatic or
  explicit lineage selection.
- **Landmark profiles** —
  [`linf.landmarks()`](https://pgajer.github.io/linf/reference/linf.landmarks.md):
  representative compositional profiles (endpoint max/min, mean) for
  each dCST.
- **ASV filtering** —
  [`filter.asv()`](https://pgajer.github.io/linf/reference/filter.asv.md):
  library-size and prevalence filtering for amplicon count matrices.
...
![Barplot of absorb-policy depth-1 dCST sizes in 763 filtered AGP gut
samples](reference/figures/readme-dcst-barplot.png)
...
MIT © 2025 Pawel Gajer. See
[LICENSE](https://pgajer.github.io/linf/LICENSE). Bundled-data sources
and upstream terms are recorded in `inst/DATA_PROVENANCE.md`.
